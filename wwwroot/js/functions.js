...
function blockuiForHome(contoller, action) {
    $.blockUI({
        message: '<h1><img src="/images/wait.gif" /></h1>',
        css: {
    ...
    window.location = window.location.origin + "/" + contoller + "/" + action;
}
...
function checkECOUserInputs(x) { //this will check for the inputs of the user
    ...
    else if (x.id == "btnTemplate") {//same thing as above
        $("#Status option").removeAttr("selected");
        $("#Status option[value='5']").attr("selected", "selected"); //but sets the status option to 0, which is Draft
    }
    ...
    var Fields = { //these are the fields that will be checked.
        ...
        "Previous Revision": $("#PreviousRevision").val(),
        "New Revision": $("#NewRevision").val(),
    ...
    for (var reqApprover in requiredApprovers) {
        if ($("select[id='" + reqApprover + "']").val() != undefined) {
...
function checkECNUserInputs(x) {
    ...
    var Fields = {
        ...
        "Description": $("#Description").val(),
        "Previous Revision": $("#CurrentFirmwareVersion").val(),
        "New Revision": $("#NewFirmwareVersion").val(),
...
$('document').ready(function () {
    ...
    $(".DataTableWaitMsg").on("click", function () {
...
$(function () {
    ...
    $("#DeviationQuantity").inputFilter(function (value) {
    ...
    $('.TemplateTable').DataTable({ //DataTable for the Template page
        order: [[4, 'desc']],
        "columnDefs": [
            ...
            { "width": '40%', "targets": [2] },
            { "width": '10%', "targets": [3] },
            { "width": '10%', "targets": [4, 5, 6] }, //sizes for the columns
            {
                "targets": [3],
